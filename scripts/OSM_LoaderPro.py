...
def buildWays(scratch,blocks):
    ...
    for fs in range(1,blocks+1):
        ...
        for node in nodefile:
            # Python3 requires bytes to string conversion
            #snode=node.rstrip('\n').split(':')
            snode=node.decode("utf-8").rstrip('\n').split(':')
        ...
        for way in unbuiltways:
            # Python3 requires bytes to string conversion
            #wayitems=way.rstrip('\n').rstrip(':').split('#')
            wayitems=way.decode("utf-8").rstrip('\n').rstrip(':').split('#')
            if len(wayitems) ==2:
                wayid=wayitems[0]
                ...
                if isAllComplete:
                    ...
                    if finalnodes[0] != finalnodes[-2]:     #start point is different to end point so its a line
                        # Python3 requires bytes instead of string
                        #builtlines.write(wayout+'\n')
                        builtlines.write(bytes(wayout+'\n', "utf-8"))
                    else:
                        # Python3 requires bytes instead of string
                        #builtareas.write(wayout+'\n')       #its an area
                        builtareas.write(bytes(wayout+'\n', "utf-8"))       #its an area
                else:
                    # Python3 requires bytes instead of string
                    #stillunbuiltways.write(wayout+'\n')
                    stillunbuiltways.write(bytes(wayout+'\n', "utf-8"))
...
for uline in inputfile:
    ...
    if element=='node':
        ...
        unbuiltways.write(bytes(waystring, "utf-8"))
        ...
        if isMultipolygon:
            ...
            unbuiltrelations.write(bytes(relstring, "utf-8"))
            if len(ftags)>0:
...
#Create error log
errorLog=open(scratchSpace+'/errors.log', 'w')
...
for lineway in builtlines:
    ...
    linewayitems=lineway.decode("utf-8").rstrip('\n').rstrip(':').split('#')
...
if 0 < errorCount:
    errorLog.write("Failed to load %d lines." % errorCount)
...
for areaway in builtareas:
    ...
    areawayitems=areaway.decode("utf-8").rstrip('\n').rstrip(':').split('#')
...
for rel in unbuiltrelations:
    ...
    areawayitems=rel.decode("utf-8").rstrip('\n').rstrip(':').split('#')
    if len(areawayitems)==2:
        shape.removeAll()
...
if 0 < errorCount:
    errorLog.write("Failed to load %d ways as areas." % errorCount)
